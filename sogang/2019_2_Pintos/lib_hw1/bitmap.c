...
struct bitmap *bitmap_expand(struct bitmap *bitmap, int size){
    struct bitmap* exp_bitmap;
    unsigned long exp_size = bitmap->bit_cnt + size;
    exp_bitmap = (struct bitmap*)malloc(sizeof *exp_bitmap);	// 그만한 공간이 있냐를 체크 중
    if(!exp_bitmap) return NULL;			// 공간없으면 만족하고 bitmap으로 사세여~
    
    exp_bitmap->bit_cnt = exp_size;
    exp_bitmap->bits = ( elem_type*)realloc(bitmap->bits, byte_cnt(exp_size)); 
    
    if(!exp_bitmap->bits) return NULL;
    free(bitmap->bits);
    free(bitmap);
    return exp_bitmap;
